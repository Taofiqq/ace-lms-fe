...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen flex bg-[#FAFAFA]">
      {/* Sidebar - common for all roles but with role-specific navigation */}
      <div className="w-64 bg-white shadow-md hidden md:block">
        <div className="p-4 border-b">
          <h1 className="text-xl font-bold text-[#000000]">ACE LMS</h1>
        </div>
      ...
      {/* Main Content - Render dashboard based on user role */}
      {user.role === "admin" && (
        <AdminDashboard user={user} handleLogout={handleLogout} />
      )}

      {user.role === "instructor" && (
        <InstructorDashboard user={user} handleLogout={handleLogout} />
      )}

      {user.role === "learner" && (
        <LearnerDashboard user={user} handleLogout={handleLogout} />
      )}
    </div>
  );
}
